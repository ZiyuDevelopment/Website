document.addEventListener("DOMContentLoaded", () => {
  ...
  document.querySelectorAll("[data-remove-item], .gg-cart-remove").forEach(button => {
    button.addEventListener("click", async event => {
      ...
      const cartItem =
        ...
        button.closest(".gg-cart-item");

      button.classList.add("is-loading");

      try {
        const cart = await changeCartLine(line, 0);
  ...
  if (thumbsScroller && thumbUp && thumbDown) {
    thumbUp.addEventListener("click", () => moveThumb(-1));
    thumbDown.addEventListener("click", () => moveThumb(1));
